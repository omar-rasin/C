...
the participant who got the i-th place. The given sequence is non-increasing (that is, for all i from 1 to n - 1 the following condition is
 fulfilled: ai ≥ ai + 1).
...
int main() {
   ...
   int tresholdScore = scores[k-1];
   int count = 0;

   for(int i=0; i<n; i++){
        if(scores[i]>= tresholdScore && scores[i]>0) {
            count++;
        }
   }
